  ...
  scrap = (siteString, processRootUrlOnly, searchStrength) => {
    return new Promise(function (resolve, reject) {
      ...
      const processSiteArray = async (
        ...
        return Promise.all(
          siteArray.map((site) =>
            processSite(site, processRootUrlOnly, searchStrength)
          )
        );
      };
        ...
        return new Promise(function (resolve, reject) {
          email
            ...
            .then((result) => {
              if (
                result &&
                typeof result.emails !== "undefined" &&
                result.emails !== null
              ) {
      ...
      processSiteArray(siteArray, processRootUrlOnly, searchStrength).then(
        (result) => {
          resolve(result);
        }
      );
